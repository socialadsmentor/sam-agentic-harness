...
A common shape: a fix lands in one code path (say, an API response), the self-review passes because it shares the same blind spot as the fix, and the same class of miss (a forgotten query column, a duplicated sandbox-vs-production path) resurfaces across several follow-up "fixes" until an actual test run - not another self-review - catches it.

The pattern: **sandbox/production path inconsistency** is a common AI-introduced regression.
