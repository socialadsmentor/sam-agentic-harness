...
// Defense in depth: contextDir comes from the on-disk registry (projects.json).
// Refuse to rmSync a value that isn't a plain directory name, in case that
// registry is ever hand-edited or corrupted.
if (!contextDir || contextDir.includes('/') || contextDir.includes('\\') || contextDir.includes('..')) {
  console.log(`ck: refusing to remove suspicious context directory "${contextDir}"`);
  process.exit(1);
}
